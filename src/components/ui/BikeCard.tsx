...
export default function BikeCard({ bike, wishlisted = false, onWishlistToggle }: Props) {
  ...
  return (
    <Link to={`/bikes/${bike.slug}`} className="group block bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-800 hover:border-red-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/10 hover:-translate-y-1">
      <div className="relative aspect-[16/10] overflow-hidden bg-neutral-800">
        ...
        <div className="absolute top-3 left-3 flex gap-2 flex-wrap">
          ...
          {bike.is_featured && <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-red-500/20 text-red-400 border border-red-500/30">Featured</span>}
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div>
            <p className="text-xs text-red-400 font-semibold uppercase tracking-wider mb-1">{bike.brand?.name}</p>
            <h3 className="text-white font-bold text-base leading-tight group-hover:text-red-300 transition-colors line-clamp-2">{bike.name}</h3>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-red-400 font-black text-lg">KES {bike.price.toLocaleString()}</p>
        ...
        <div className="flex items-center gap-1 mt-3 pt-3 border-t border-neutral-800/50">
          <Eye className="w-3.5 h-3.5 text-gray-600" />
          <span className="text-xs text-gray-600">{bike.views} views</span>
          <span className="ml-auto text-xs font-medium text-red-400 group-hover:underline">View Details →</span>
        </div>
      </div>
    </Link>
  );
