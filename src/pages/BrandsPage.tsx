    ...
    <main className="bg-zinc-950 min-h-screen pt-24 pb-20">
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-3xl mx-auto px-4">
          <p className="text-blue-400 font-semibold text-sm tracking-wider uppercase mb-3">World-Class Manufacturers</p>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{[...Array(4)].map((_, i) => <div key={i} className="bg-zinc-900 rounded-2xl h-64 animate-pulse border border-zinc-800" />)}</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {brands.map(brand => (
              <div key={brand.id} id={brand.name.toLowerCase()} className="bg-zinc-900 border border-zinc-800 hover:border-blue-500/40 rounded-2xl p-8 group transition-all hover:shadow-xl hover:shadow-blue-500/5 hover:-translate-y-1 duration-300">
                <div className="flex items-start gap-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-2xl font-black text-white group-hover:text-blue-300 transition-colors">{brand.name}</h2>
                      <span className="px-2 py-0.5 bg-zinc-800 text-gray-400 text-xs rounded-full">{brand.country_of_origin}</span>
                    </div>
                    <p className="text-gray-400 text-sm leading-relaxed mb-4">{brand.description}</p>
                    <div className="flex items-center gap-3">
                      <Link to={`/bikes?brand=${brand.id}`} className="flex items-center gap-1.5 text-sm font-semibold text-blue-400 hover:text-blue-300 transition-colors">View Models <ArrowRight className="w-4 h-4" /></Link>
      ...
      <section className="py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 px-8 py-4 bg-blue-500 hover:bg-blue-400 text-white font-bold rounded-xl transition-all hover:shadow-lg hover:shadow-blue-500/30">Get in Touch <ArrowRight className="w-5 h-5" /></Link>
        </div>
      </section>
    </main>
  );
