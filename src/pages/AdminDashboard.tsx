...
export default function AdminDashboard() {
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
      {/* Admin Top Bar */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#111]/95 backdrop-blur-xl border-b border-white/5 h-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-yellow-500/20 to-yellow-600/10 border border-yellow-500/20 rounded-lg flex items-center justify-center">
              <LayoutDashboard className="w-5 h-5 text-yellow-400" />
            </div>
            <div>
              <p className="text-yellow-400 font-display font-bold text-xs tracking-wider uppercase leading-tight">Admin Panel</p>
              <p className="text-gray-500 text-xs truncate max-w-[200px]">{user.email}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link to="/" className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white transition-colors px-3 py-2 bg-white/5 rounded-lg">
...
function StatCard({ label, value, icon: Icon, color, sub }: { label: string; value: number | string; icon: React.ElementType; color: string; sub?: string }) {
  return (
    <div className="bg-[#111] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
      ...
      <p className="text-3xl font-display font-black text-white mb-0.5">{value}</p>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2.5 py-1 text-xs font-bold rounded-full border capitalize ${map[status] || 'bg-white/5 text-gray-400 border-white/10'}`}>
      {status}
    </span>
  );
}
...
function EmptyState({ icon: Icon, title, desc, action }: { icon: React.ElementType; title: string; desc: string; action?: React.ReactNode }) {
  return (
    <div className="text-center py-16 bg-[#111] rounded-2xl border border-white/10">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-display font-black text-white tracking-tight">Dashboard Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
          <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-red-400" /> Inventory by Type</h2>
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-display font-bold text-white tracking-wide flex items-center gap-2"><Calendar className="w-5 h-5 text-yellow-400" /> Recent Bookings</h2>
          ...
          <div className="space-y-3">
            {recentBookings.length === 0 ? (
              ...
              <div key={b.id} className="flex items-center justify-between gap-3 py-2 border-b border-white/10 last:border-0">
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-display font-bold text-white tracking-wide flex items-center gap-2"><MessageSquare className="w-5 h-5 text-orange-400" /> Recent Inquiries</h2>
          ...
          <div className="space-y-3">
            {recentInquiries.length === 0 ? (
              ...
              <div key={i.id} className="flex items-start justify-between gap-3 py-2 border-b border-white/10 last:border-0">
                <div className="min-w-0">
      ...
      {/* Quick Actions */}
      <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
        <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4">Quick Actions</h2>
...
export function AdminBikes() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-display font-black text-white tracking-tight">Bike Inventory</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search bikes..."
            className="w-full pl-9 pr-4 py-2.5 bg-[#111] border border-white/10 text-white text-sm rounded-xl focus:outline-none focus:border-red-500/60" />
        </div>
        <div className="flex gap-2">
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            className="px-3 py-2.5 bg-[#111] border border-white/10 text-white text-sm rounded-xl focus:outline-none focus:border-red-500/60 cursor-pointer appearance-none">
          ...
          <select value={filterType} onChange={e => setFilterType(e.target.value)}
            className="px-3 py-2.5 bg-[#111] border border-white/10 text-white text-sm rounded-xl focus:outline-none focus:border-red-500/60 cursor-pointer appearance-none capitalize">
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 px-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-3xl shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              ...
              <button onClick={() => { setShowForm(false); setEditingBike(null); }} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/5 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={saveBike} className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={key}>
                  ...
                  <input type={type} required={required} value={(form as Record<string, string | number | boolean>)[key] as string}
                    ...
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:outline-none focus:border-red-500/60 transition-colors" />
                </div>
              ))}
              <div>
                ...
                <select value={form.brand_id} onChange={e => setForm(f => ({ ...f, brand_id: e.target.value }))}
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:outline-none focus:border-red-500/60">
              ...
              <div>
                ...
                <select value={form.bike_type} onChange={e => setForm(f => ({ ...f, bike_type: e.target.value }))}
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:outline-none focus:border-red-500/60 capitalize">
              ...
              <div className="sm:col-span-2 lg:col-span-3">
                ...
                <textarea value={form.description} rows={3} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:outline-none focus:border-red-500/60 resize-none" />
              </div>
              <div className="sm:col-span-2 lg:col-span-3">
              ...
              <div className="sm:col-span-2 lg:col-span-3 flex gap-3 pt-2">
                ...
                <button type="button" onClick={() => { setShowForm(false); setEditingBike(null); }}
                  className="px-6 py-2.5 border border-white/10 text-gray-300 hover:text-white font-semibold rounded-xl text-sm transition-colors">
                  Cancel
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 border border-white/10 text-gray-300 hover:text-white font-semibold rounded-xl text-sm transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Bike List */}
      {loading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="bg-[#111] rounded-2xl h-20 animate-pulse border border-white/10" />)}</div>
      ) : bikes.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          {bikes.map(bike => (
            <div key={bike.id} className="bg-[#111] border border-white/10 hover:border-white/10 rounded-2xl p-4 flex items-center gap-4 transition-colors">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button onClick={() => toggleFeatured(bike.id, bike.is_featured)} title={bike.is_featured ? 'Unfeature' : 'Feature'}
                  className="p-2 text-gray-400 hover:text-yellow-400 bg-white/5 rounded-lg transition-colors">
                  {bike.is_featured ? <ToggleRight className="w-4 h-4 text-yellow-400" /> : <ToggleLeft className="w-4 h-4" />}
                </button>
                <Link to={`/bikes/${bike.slug}`} className="p-2 text-gray-400 hover:text-red-400 bg-white/5 rounded-lg transition-colors">
                  <Eye className="w-4 h-4" />
                </Link>
                <button onClick={() => openEdit(bike)} className="p-2 text-gray-400 hover:text-emerald-400 bg-white/5 rounded-lg transition-colors">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={() => toggleStatus(bike.id, bike.status)} title={bike.status === 'available' ? 'Mark Sold' : 'Mark Available'}
                  className="p-2 text-gray-400 hover:text-emerald-400 bg-white/5 rounded-lg transition-colors">
                  {bike.status === 'available' ? <XCircle className="w-4 h-4" /> : <CheckCircle className="w-4 h-4" />}
                </button>
                <button onClick={() => setDeleteConfirm(bike.id)} className="p-2 text-gray-400 hover:text-red-400 bg-white/5 rounded-lg transition-colors">
...
export function AdminBookings() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-black text-white tracking-tight">Test Ride Bookings</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              ...
              <button onClick={() => setSelectedBooking(null)} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/5"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              {selectedBooking.notes && (
                <div>
                  ...
                  <p className="text-gray-300 text-sm bg-white/5 rounded-xl p-3">{selectedBooking.notes}</p>
                </div>
              )}
              <div>
                ...
                <button onClick={saveAdminNotes} disabled={saving}
                  className="mt-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5">
              ...
              <div className="flex gap-2 pt-3 border-t border-white/10">
                ...
                {(selectedBooking.status === 'cancelled' || selectedBooking.status === 'completed') && (
                  <button onClick={() => updateStatus(selectedBooking.id, 'pending')} className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 text-gray-300 font-bold rounded-xl text-sm transition-colors">Reopen</button>
                )}
              </div>
            </div>
          </div>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="bg-[#111] rounded-2xl h-24 animate-pulse border border-white/10" />)}</div>
      ) : bookings.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          {bookings.map(b => (
            <div key={b.id} className="bg-[#111] border border-white/10 hover:border-white/10 rounded-2xl p-5 transition-colors cursor-pointer" onClick={() => openDetail(b)}>
...
export function AdminInquiries() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-black text-white tracking-tight">Customer Inquiries</h1>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="bg-[#111] rounded-2xl h-32 animate-pulse border border-white/10" />)}</div>
      ) : inquiries.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {inquiries.map(inq => (
            <div key={inq.id} className="bg-[#111] border border-white/10 hover:border-white/10 rounded-2xl p-5 transition-colors">
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {inq.status === 'new' && (
                    <button onClick={() => updateStatus(inq.id, 'read')} className="text-xs text-gray-400 hover:text-white border border-white/10 px-3 py-1 rounded-lg transition-colors">Mark Read</button>
                  )}
                  {inq.status !== 'closed' && inq.status !== 'new' && (
                    <button onClick={() => updateStatus(inq.id, 'closed')} className="text-xs text-gray-400 hover:text-white border border-white/10 px-3 py-1 rounded-lg transition-colors">Close</button>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setReplyingTo(null); setReplyText(''); }}
                      className="px-4 py-2 border border-white/10 text-gray-300 hover:text-white text-xs font-semibold rounded-lg transition-colors">Cancel</button>
                  </div>
                </div>
              ) : (
...
export function AdminCustomers() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-black text-white tracking-tight">Customers ({profiles.length})</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 px-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              ...
              <button onClick={() => setSelectedCustomer(null)} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/5"><X className="w-5 h-5" /></button>
            </div>
                ...
                {customerInquiries.length === 0 ? <p className="text-gray-500 text-sm">No inquiries.</p> : (
                  <div className="space-y-2">
                    {customerInquiries.map(i => (
                      <div key={i.id} className="flex items-start justify-between bg-white/5 rounded-xl p-3">
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="bg-[#111] rounded-xl h-16 animate-pulse border border-white/10" />)}</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(p => (
            <div key={p.id} className="bg-[#111] border border-white/10 hover:border-white/10 rounded-xl p-4 flex items-center gap-4 transition-colors cursor-pointer" onClick={() => openCustomerDetail(p)}>
...
export function AdminBlogPosts() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-display font-black text-white tracking-tight">Blog Posts</h1>
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl transition-colors text-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 px-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              ...
              <button onClick={() => { setShowForm(false); setEditingPost(null); }} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/5"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={savePost} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="button" onClick={() => { setShowForm(false); setEditingPost(null); }}
                  className="px-6 py-2.5 border border-white/10 text-gray-300 hover:text-white font-semibold rounded-xl text-sm transition-colors">Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="bg-[#111] rounded-2xl h-20 animate-pulse border border-white/10" />)}</div>
      ) : posts.length === 0 ? (
        ...
        <div className="space-y-3">
          {posts.map(post => (
            <div key={post.id} className="bg-[#111] border border-white/10 hover:border-white/10 rounded-2xl p-4 flex items-center gap-4 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border ${post.is_published ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-white/5 text-gray-400 border-white/10'}`}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button onClick={() => openEdit(post)} className="p-2 text-gray-400 hover:text-red-400 bg-white/5 rounded-lg transition-colors">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={() => deletePost(post.id)} className="p-2 text-gray-400 hover:text-red-400 bg-white/5 rounded-lg transition-colors">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            </div>
...
export function AdminSettings() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-black text-white tracking-tight">Settings</h1>

      <div className="flex gap-1 bg-[#111] rounded-xl p-1 w-fit flex-wrap">
      ...
      {activeTab === 'access' ? (
        <div className="space-y-6">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 bg-gradient-to-br from-yellow-500/20 to-yellow-600/10 border border-yellow-500/20 rounded-xl flex items-center justify-center">
                <Shield className="w-5 h-5 text-yellow-400" />
              </div>
              <div>
                <h2 className="text-lg font-display font-bold text-white tracking-wide">Grant Admin Access</h2>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <input type="email" required value={grantEmail} onChange={e => setGrantEmail(e.target.value)} placeholder="user@example.com"
                    className="flex-1 px-4 py-3 bg-white/5 border border-white/10 text-white placeholder-gray-500 text-sm rounded-xl focus:outline-none focus:border-yellow-500 transition-colors" />
                  <button type="submit" disabled={accessAction === 'granting' || !grantEmail.trim()}
                    className="flex items-center gap-2 px-5 py-3 bg-yellow-500 hover:bg-yellow-400 disabled:bg-yellow-500/50 text-[#0a0a0a] font-bold rounded-xl transition-colors text-sm whitespace-nowrap">
                    {accessAction === 'granting' ? <><span className="w-4 h-4 border-2 border-[#0a0a0a]/30 border-t-[#0a0a0a] rounded-full animate-spin" />Granting...</> : <><Shield className="w-4 h-4" />Grant Admin</>}
                  </button>
                </div>
              </div>
            </form>
          ...
          {/* Current Admin Users */}
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
            <h2 className="text-lg font-display font-bold text-white tracking-wide mb-2">Manage Admin Users</h2>
            <p className="text-gray-400 text-sm mb-5">Revoke admin access from any user. You cannot revoke your own access.</p>

            {accessLoading ? (
              <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="bg-white/5 rounded-xl h-14 animate-pulse" />)}</div>
            ) : adminUsers.length === 0 ? (
              ...
              <div className="space-y-3">
                {adminUsers.map(admin => (
                  <div key={admin.id} className="flex items-center justify-between gap-4 bg-white/5 rounded-xl p-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black flex-shrink-0 ${admin.role === 'admin' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-white/10 text-gray-400'}`}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {admin.email === currentUser?.email && (
                        <span className="text-xs text-gray-500 px-3 py-2 bg-white/10/50 rounded-lg">You</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {/* How It Works */}
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
            <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4">How Admin Access Works</h2>
        ...
        <form onSubmit={handleSave} className="bg-[#111] border border-white/10 rounded-2xl p-6">
          {activeTab === 'general' && (
            <div className="space-y-4 max-w-lg">
              <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4">General Settings</h2>
              {[
                ...
                <div key={key}>
                  ...
                  <input type="text" value={(settings as Record<string, string | boolean>)[key] as string}
                    ...
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 text-white text-sm rounded-xl focus:outline-none focus:border-red-500/60 transition-colors" />
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-4 max-w-lg">
              <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4">Notification Settings</h2>
              {[
                ...
                <div key={key} className="flex items-center justify-between gap-4 py-3 border-b border-white/10 last:border-0">
                  <div>
          ...
          {activeTab === 'seo' && (
            <div className="space-y-4 max-w-lg">
              <h2 className="text-lg font-display font-bold text-white tracking-wide mb-4">SEO Settings</h2>
              <div>
              ...
              <div>
                ...
                <textarea value={settings.meta_description} onChange={e => setSettings(s => ({ ...s, meta_description: e.target.value }))} rows={3}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 text-white text-sm rounded-xl focus:outline-none focus:border-red-500/60 transition-colors resize-none" />
              </div>
            </div>
          )}
