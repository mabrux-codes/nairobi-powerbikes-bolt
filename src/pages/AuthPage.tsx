...
export default function AuthPage() {
  ...
  return (
    <main className="bg-neutral-950 min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-end p-12 pb-16">
          <Link to="/" className="flex items-center gap-2 mb-auto mt-8">
            <div className="w-10 h-10 bg-red-600 rounded-lg flex items-center justify-center"><Bike className="w-6 h-6 text-white" /></div>
            <div><span className="block text-white font-black text-lg leading-tight">NAIROBI</span><span className="block text-red-400 font-bold text-xs tracking-[0.2em] leading-tight">POWERBIKES</span></div>
          </Link>
          <div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-md">
          <Link to="/" className="flex items-center gap-2 mb-10 lg:hidden">
            <div className="w-9 h-9 bg-red-600 rounded-lg flex items-center justify-center"><Bike className="w-5 h-5 text-white" /></div>
          ...
          <div className="flex bg-neutral-900 rounded-xl p-1 mb-8">
            <button onClick={() => setMode('signin')} className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all ${mode === 'signin' ? 'bg-red-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Sign In</button>
            <button onClick={() => setMode('signup')} className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all ${mode === 'signup' ? 'bg-red-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Create Account</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <input type="text" required value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="John Doe"
                    className="w-full px-4 py-3.5 bg-neutral-900 border border-neutral-700 text-white placeholder-gray-500 text-sm rounded-xl focus:outline-none focus:border-red-500" />
                </div>
                <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} required value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="--------" minLength={6}
                  className="w-full px-4 py-3.5 bg-neutral-900 border border-neutral-700 text-white placeholder-gray-500 text-sm rounded-xl focus:outline-none focus:border-red-500 pr-12" />
            ...
            <button type="submit" disabled={loading}
              className="w-full py-4 bg-red-600 hover:bg-red-500 disabled:bg-red-600/50 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 mt-2">
