  ...
  return (
    <main className="bg-[#0a0a0a] min-h-screen pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-14">
        <p className="text-red-400 font-display font-bold text-xs tracking-[0.2em] uppercase mb-2">Get in Touch</p>
        <h1 className="text-5xl font-display font-black tracking-tight text-white mb-3">Contact Us</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-5 gap-10">
        <div className="lg:col-span-2 space-y-5">
          {[
            ...
            <div key={title} className="card-premium p-6">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-center flex-shrink-0"><Icon className="w-5 h-5 text-red-400" /></div>
                <div>
                  <h3 className="text-white font-display font-bold mb-2">{title}</h3>
                  {content && <p className="text-gray-400 text-sm whitespace-pre-line">{content}</p>}
                  {rest.link && <a href={rest.link.href} target="_blank" rel="noopener noreferrer" className="text-red-400 hover:text-red-300 text-sm font-semibold mt-2 inline-block">{rest.link.label}</a>}
          ...
          <div className="card-premium p-6">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-center flex-shrink-0"><Clock className="w-5 h-5 text-red-400" /></div>
              <div>
                <h3 className="text-white font-display font-bold mb-3">Business Hours</h3>
          ...
          <div className="card-premium p-6">
            <h3 className="text-white font-display font-bold mb-4">Connect With Us</h3>
            ...
            <div className="flex gap-3">
              {[{ icon: Facebook, label: 'Facebook' }, { icon: Instagram, label: 'Instagram' }, { icon: Youtube, label: 'YouTube' }, { icon: Twitter, label: 'Twitter' }].map(({ icon: Icon, label }) => (
                <a key={label} href="#" aria-label={label} className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-red-600 hover:border-red-600 transition-all duration-300"><Icon className="w-4 h-4" /></a>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-8">
            <h2 className="text-2xl font-display font-black tracking-tight text-white mb-6">Send a Message</h2>
            {sent ? (
              <div className="text-center py-10">
                <div className="w-16 h-16 bg-emerald-500/20 border border-emerald-500/30 rounded-full flex items-center justify-center mx-auto mb-4"><span className="text-emerald-400 text-3xl">&#10003;</span></div>
                <h3 className="text-white font-display font-bold text-xl mb-2">Message Received!</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-red-400 font-display font-bold text-xs tracking-[0.2em] uppercase mb-2">Email *</label>
                  <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="you@example.com"
                    className="input-premium" />
                </div>
                <div>
                  <label className="block text-red-400 font-display font-bold text-xs tracking-[0.2em] uppercase mb-2">Message *</label>
