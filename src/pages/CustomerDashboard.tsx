        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="flex items-center gap-2 group">
              <div className="w-9 h-9 bg-red-600 rounded-lg flex items-center justify-center group-hover:bg-red-400 transition-colors">
                <Bike className="w-5 h-5 text-white" />
              </div>
              <div>
                <span className="block text-white font-black text-sm leading-tight">NAIROBI</span>
                <span className="block text-red-400 font-bold text-[10px] tracking-[0.15em] leading-tight">POWERBIKES</span>
              </div>
            </Link>
            <span className="hidden sm:block text-gray-500 text-xs ml-2 pl-2 border-l border-neutral-700">My Dashboard</span>
          </div>
          <div className="flex items-center gap-3">
              ...
              <nav className="space-y-1">
                {sideNav.map(({ to, label, icon: Icon, end }) => (
                  <NavLink key={to} to={to} end={end}
                    className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-red-600/20 text-red-400 border border-red-600/30' : 'text-gray-400 hover:text-white hover:bg-neutral-800'}`}>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: 'Test Ride Bookings', value: stats.bookings, to: '/dashboard/bookings', color: 'text-red-400' },
          { label: 'My Inquiries', value: stats.inquiries, to: '/dashboard/inquiries', color: 'text-emerald-400' },
        ].map(({ label, value, to, color }) => (
          <Link key={to} to={to} className="bg-neutral-900 border border-neutral-800 hover:border-neutral-600 rounded-2xl p-6 transition-all">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link to="/bikes" className="flex items-center gap-3 px-4 py-3 bg-red-600/10 border border-red-600/20 hover:border-red-600/40 rounded-xl text-red-400 font-semibold text-sm transition-all">Browse Available Bikes →</Link>
          <Link to="/booking" className="flex items-center gap-3 px-4 py-3 bg-neutral-800 hover:bg-neutral-700 rounded-xl text-white font-semibold text-sm transition-all">Book a Test Ride →</Link>
          <Link to="/contact" className="flex items-center gap-3 px-4 py-3 bg-neutral-800 hover:bg-neutral-700 rounded-xl text-white font-semibold text-sm transition-all">Contact Our Team →</Link>
          <Link to="/dashboard/profile" className="flex items-center gap-3 px-4 py-3 bg-neutral-800 hover:bg-neutral-700 rounded-xl text-white font-semibold text-sm transition-all">Update Profile →</Link>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      {loading ? <p className="text-gray-400">Loading...</p> : items.length === 0 ? (
        <div className="text-center py-16 bg-neutral-900 rounded-2xl border border-neutral-800">
          ...
          <Link to="/bikes" className="mt-4 inline-block text-red-400 hover:text-red-300 font-semibold text-sm">Browse Bikes →</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {items.map(item => (
            <div key={item.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden flex gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-red-400 font-bold text-sm mt-1">KES {item.bike?.price?.toLocaleString()}</p>
                <div className="flex gap-2 mt-2">
                  <Link to={`/bikes/${item.bike?.slug}`} className="text-xs text-red-400 hover:text-red-300 font-semibold">View →</Link>
    ...
    <div>
      ...
      {loading ? <p className="text-gray-400">Loading...</p> : bookings.length === 0 ? (
        <div className="text-center py-16 bg-neutral-900 rounded-2xl border border-neutral-800">
          ...
          <Link to="/booking" className="mt-4 inline-block text-red-400 hover:text-red-300 font-semibold text-sm">Book a Test Ride →</Link>
        </div>
      ) : (
          ...
          {bookings.map(b => (
            <div key={b.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`px-3 py-1 text-xs font-bold rounded-full border capitalize flex-shrink-0 ${statusColors[b.status] || 'bg-neutral-800 text-gray-400 border-neutral-700'}`}>{b.status}</span>
              </div>
            </div>
          ))}
        </div>
...
export function DashboardInquiries() {
  ...
  const statusColors: Record<string, string> = { new: 'bg-red-600/20 text-red-400 border-red-600/30', read: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', replied: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30', closed: 'bg-neutral-500/20 text-neutral-400 border-neutral-500/30' };
    ...
    <div>
      ...
      {loading ? <p className="text-gray-400">Loading...</p> : inquiries.length === 0 ? (
        <div className="text-center py-16 bg-neutral-900 rounded-2xl border border-neutral-800">
          ...
          <Link to="/contact" className="mt-4 inline-block text-red-400 hover:text-red-300 font-semibold text-sm">Send an Inquiry →</Link>
        </div>
      ) : (
        <div className="space-y-4">
          {inquiries.map(inq => (
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
        <form onSubmit={handleSave} className="space-y-4 max-w-lg">
          ...
          <div>
            ...
            <input type="email" value={user?.email || ''} disabled className="w-full px-4 py-3 bg-neutral-800/50 border border-neutral-700 text-gray-500 text-sm rounded-xl cursor-not-allowed" />
          </div>
          <button type="submit" disabled={saving} className="px-8 py-3 bg-red-600 hover:bg-red-400 disabled:bg-red-600/50 text-white font-bold rounded-xl transition-colors flex items-center gap-2">
